  ...
  export default {
    ...
    computed: {
      ...
      routes() {
        ...
        // 权限路由组合
        const fuctionList = localStorage.getItem('functionList') ? JSON.parse(
          localStorage.getItem('functionList')) : []
        const leftMenu = fuctionList.filter(
          item => item.functionType === 1 && item.operationType != 'node' && item.operationType
            != 'icon')
        let authRouter = []
        // 组合左侧一级路由
        for (let i = 0; i < showRoutesArr.length; i++) {
          for (let j = 0; j < leftMenu.length; j++) {
            if (showRoutesArr[i].meta.title === leftMenu[j].functionLabel
              && (leftMenu[j].operationType == 'stretch' || leftMenu[j].operationType
                == 'leaf_list')) {
        ...
        for (let i = 0; i < authRouter.length; i++) {
          ...
          for (let j = 0; j < authRouter[i].children.length; j++) {
            for (let k = 0; k < leftMenu.length; k++) {
              if (authRouter[i].children[j].meta.title === leftMenu[k].functionLabel
                && (leftMenu[k].operationType === 'list' || leftMenu[k].operationType
                  === 'leaf_list' || leftMenu[k].operationType === 'parent_list'
                  || leftMenu[k].operationType === 'get' || leftMenu[k].operationType === 'put')) {
        ...
        for (let i = 0; i < authRouter.length; i++) {
          hasThirdRoute = authRouter[i].children ? authRouter[i].children.filter(
            item => item.children).length ? authRouter[i].children.filter(item => item.children)
            : '' : ''
            ...
            for (let j = 0; j < hasThirdRoute.length; j++) {
              ...
              for (let k = 0; k < hasThirdRoute[j].children.length; k++) {
                for (let l = 0; l < leftAuthRouter.length; l++) {
                  ...
                  if (hasThirdRoute[j].children[k].path === leftAuthRouter[l].route.split('/')[len
                  - 1]) {
