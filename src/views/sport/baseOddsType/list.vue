<template>
  <three-level-route>
        ...
        <el-col :span="6">
          <el-form-item size="small">
            <el-select v-model="queryParam.type">
              <el-option label="指数类型" value=""></el-option>
              <el-option
    ...
    <el-table :data="tableData" border height="600" style="margin-top: 20px"
      ...
      <el-table-column align="center" label="指数类型编号" prop="id"/>
      <el-table-column align="center" label="指数类型" prop="type">
...
export default {
  ...
  methods: {
    getPage() {
      baseOddsTypeApi.page(this.queryParam).then(res => {
        if (res.subCode === 1000) {
    ...
    goDetail(id, type) {
      // *** 根据真实路径配置地址
      this.$router.push({ path: '/sport/baseOddsType/detail', query: { id, type } })
    },
    goDel(id) {
      baseOddsTypeApi.delById(id).then(res => {
        if (res.subCode === 1000) {
    ...
    changeStatus(id, dataStatus) {
      baseOddsTypeApi.changeStatus({ id, dataStatus }).then(res => {
        if (res.subCode === 1000) {
          this.$message.success(res.subMsg)
        } else {
          this.$message.error(res.subMsg)
    ...
    batchdelete() {
      ...
      this.$confirm('是否继续?', '提示', {
        ...
        baseOddsTypeApi.batchdelete(this.ids).then(res => {
          if (res.subCode === 1000) {
            this.$message.success(res.subMsg)
            this.getPage()
          } else {
    ...
    exportHandle() {
      ...
      getExport('/gw/op/v1/baseOddsType/export', data, 'post', '指数类型表列表').then(() => {
        this.$emit('refresh')
      })
    },
    resetHandle() {
