<template>
  <three-level-route>
      ...
      <el-row class="query-form">
        <el-col :span="6">
          <el-form-item size="small">
            <el-input v-model.trim="queryParam.oddsId" placeholder="指数编号"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item size="small">
            <el-input v-model.trim="queryParam.userId" placeholder="用户编号"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item size="small">
          ...
          <el-form-item size="small">
            <div>
              <el-input
                style="width: 47%"
                v-input-validation
                v-model.trim="queryParam.orderPriceFrom"
                placeholder="订单金额开始">
              </el-input>
              <span> - </span>
              <el-input
                style="width: 47%"
                v-input-validation
                v-model.trim="queryParam.orderPriceTo"
                placeholder="订单金额结束">
              </el-input>
            </div>
          </el-form-item>
        ...
        <el-col :span="6">
          <el-form-item size="small">
            <el-select v-model="queryParam.status">
              <el-option label="体育订单状态：1已发起，2未生效，3已生效，4胜，5平，6负"
                         value=""></el-option>
              <el-option
          ...
          <el-form-item size="small">
            <div>
              <el-input
                style="width: 47%"
                v-input-validation
                v-model.trim="queryParam.resPriceFrom"
                placeholder="结果金额开始">
              </el-input>
              <span> - </span>
              <el-input
                style="width: 47%"
                v-input-validation
                v-model.trim="queryParam.resPriceTo"
                placeholder="结果金额结束">
              </el-input>
            </div>
          </el-form-item>
    ...
    <el-table height="600" style="margin-top: 20px" border :data="tableData"
      ...
      <el-table-column align="center" prop="id" label="体育订单编号"/>
      <el-table-column align="center" prop="oddsId" label="指数编号"/>
      <el-table-column align="center" prop="userId" label="用户编号"/>
      <el-table-column align="center" prop="odds" label="指数"/>
      <el-table-column align="center" prop="orderPrice" label="订单金额"/>
      <el-table-column align="center" prop="status"
                       label="体育订单状态：1已发起，2未生效，3已生效，4胜，5平，6负">
        <template slot-scope="scope">{{ scope.row.status | dictToDescTypeValue(64) }}</template>
      </el-table-column>
      <el-table-column align="center" prop="confirmTime" label="确认时间">
        <template slot-scope="scope">{{
            scope.row.confirmTime | formateTime('{y}-{m}-{d} {h}:{i}')
          }}
        </template>
      </el-table-column>
      <el-table-column align="center" prop="resPrice" label="结果金额"/>
...
export default {
  ...
  methods: {
    confirmTimeChange() {
      if (this.confirmTime) {
        this.queryParam.confirmTimeFrom = this.confirmTime[0]
        this.queryParam.confirmTimeTo = this.confirmTime[1]
      } else {
        this.queryParam.confirmTimeFrom = null
        this.queryParam.confirmTimeTo = null
      }
    },
    getPage() {
      sportOrderApi.page(this.queryParam).then(res => {
        if (res.subCode === 1000) {
    ...
    goDetail(id, type) {
      // *** 根据真实路径配置地址
      this.$router.push({ path: '/sportOrder/list/detail', query: { id, type } })
    },
    goDel(id) {
      sportOrderApi.delById(id).then(res => {
        if (res.subCode === 1000) {
    ...
    changeStatus(id, dataStatus) {
      sportOrderApi.changeStatus({ id, dataStatus }).then(res => {
        if (res.subCode === 1000) {
          this.$message.success(res.subMsg)
        } else {
          this.$message.error(res.subMsg)
    ...
    batchdelete() {
      ...
      this.$confirm('是否继续?', '提示', {
        ...
        sportOrderApi.batchdelete(this.ids).then(res => {
          if (res.subCode === 1000) {
            this.$message.success(res.subMsg)
            this.getPage()
          } else {
    ...
    exportHandle() {
      ...
      getExport('/gw/op/v1/sportOrder/export', data, 'post', '体育订单表列表').then(() => {
        this.$emit('refresh')
      })
    },
    resetHandle() {
