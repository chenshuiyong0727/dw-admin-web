  ...
  <three-level-route>
    ...
    >
      ...
      <el-table-column prop="functionType" label="权限类型">
        <template slot-scope="scope">{{ scope.row.functionType | dictToDescTypeValue(8) }}
        </template>
      </el-table-column>
      <el-table-column prop="operationType" width="100" label="操作类型">
        <template slot-scope="scope">{{ scope.row.operationType | dictToDescTypeValue(9) }}
        </template>
      </el-table-column>
    ...
    <copy-data-dialog v-if="isShowDialog1" :ids="ids" :keyword="keyword" :oldName="oldName"
                      @reload2="reload2" @reload1="reload1" @closeDialog1="closeDialog1"/>
    <add-data-dialog v-if="isShowDialog" :modifyId="modifyId" :systemId="queryParam.systemId"
                     :pData="pData" :isRoot="isRoot" @refreshPage="refreshPage"
  ...
  export default {
    ...
    methods: {
      ...
      listSysDict() {
        ...
        this.systemList = localStorage.getItem('systemList') ? JSON.parse(
      ...
      goCopy(val) {
        this.$confirm('是否继续?', '提示', {
          ...
          systemContainerApi.goCopy({ systemId: this.queryParam.systemId, id: val.id }).then(
            res => {
              if (res.subCode === 1000) {
                this.refreshPage(val.pid)
              } else {
