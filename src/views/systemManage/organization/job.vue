  ...
  <three-level-route>
    <el-form ref="form">
      <el-row class="query-form">
        ...
        <el-col :span="6">
          <el-cascader ref="owner" placeholder="查询归属部门" v-model="departmentIdVal" :options="options"
                       :props="props" size="small" :show-all-levels="false"></el-cascader>
          <!-- <el-form-item label="" size="small">
      ...
      <el-row v-if="buttonPermissionArr.searchBtn && buttonPermissionArr.searchBtn.length"
        ...
        <el-button v-permission:[buttonPermissionArr.searchBtn]="['查询']" type="primary" size="small"
                   style="margin-right: 10px" @click="pageListOpPost">查询
        </el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      pageListOpPost() {
        this.queryParam.departmentId = this.departmentIdVal.length
          ? this.departmentIdVal[this.departmentIdVal.length - 1] : ''
      ...
      updateStatus(id, dataStatus) {
        this.$confirm('是否确认进行此状态修改?', '提示', {
          ...
          systemContainerApi.updatePostStatus(
