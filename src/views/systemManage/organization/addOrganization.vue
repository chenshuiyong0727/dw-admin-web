<template>
  <div class="page-container">
    <el-row class="form-flex">
      ...
      <el-col :span="9" :offset="1"><el-radio v-model="paramsObj.levelOne" label="1">一级部门</el-radio></el-col>
      <el-col :span="9"><el-radio v-model="paramsObj.levelOne" label="0">非一级部门</el-radio></el-col>
    </el-row>
    ...
    <el-row>
      ...
      <el-col :span="18" :offset="1"><el-input maxlength="255" v-model="paramsObj.description" type="textarea" :rows="3"></el-input></el-col>
    </el-row>
    ...
    <department-tree-dialog v-if="isShowDialog" :departmentId="paramsObj.superiorDepartment" :departmentTreeData="departmentTreeData" :pageType="pageType" @getDepartmentMsg="getDepartmentMsg" @closDialog="closeDialog" />
  </div>
</template>
<script>
  ...
  methods: {
    ...
    confirmHandle() {
      ...
      if (!this.validNullArray.length) {
        if (this.modifyId) {
          // 修改接口
          systemContainerApi.updateDepartments({ ...this.paramsObj, id: this.modifyId }).then(res => {
            if (res.subCode === 1000) {
              this.$message({
                message: '修改成功，即将返回',
                type: 'success'
