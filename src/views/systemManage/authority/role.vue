  ...
  <three-level-route>
    <el-form ref="form">
      <el-row class="query-form">
        ...
        <el-col :span="6">
          <el-form-item label="" size="small">
            <el-input v-model.trim="queryParam.roleName" maxlength="15"
                      placeholder="查询权限角色"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="" size="small">
      ...
      <el-row v-if="buttonPermissionArr.searchBtn && buttonPermissionArr.searchBtn.length"
        ...
        <el-button v-permission:[buttonPermissionArr.searchBtn]="['查询']" type="primary" size="small"
                   style="margin-right: 10px" @click="getRolesList">查询
        </el-button>
      ...
      <el-table-column align="center" label="操作" width="200"
        ...
        <template slot-scope="scope">
          <el-button type="text" v-if="scope.row.dataStatus == 1"
                     ...
                     @click="modifyHandle(scope.row.id, scope.row.systemId)">修改
          </el-button>
          <el-button type="text" v-permission:[buttonPermissionArr.listBtn]="['查看关联']"
                     @click="showDialog(scope.row.id)">查看关联
          </el-button>
    ...
    <linked-account-dialog v-if="isShowDialog" @closDialog="closDialog"
                           :roleId="roleId"></linked-account-dialog>
  </three-level-route>
</template>
    ...
    methods: {
      ...
      updateStatus(id, dataStatus) {
        this.$confirm('是否确认进行此状态修改?', '提示', {
          ...
          systemContainerApi.updateRoleStatus(
            { id, dataStatus: dataStatus == 0 ? 1 : 0, userId }).then(res => {
            if (res.subCode === 1000) {
              this.$message({
                type: 'success',
                message: '状态修改成功'
