      ...
      <el-row class="query-form">
        <el-col :span="6">
          <el-form-item size="small">
            <el-input v-model.trim="queryParam.keyword" placeholder="关键词（货号、商品名、价格）"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item size="small">
    ...
    <el-table :row-style="{height:'40px'}"
      ...
      <el-table-column align="center" width="100" prop="actNo" label="货号">
        <template slot-scope="scope">
          <a :style="scope.row.goodsId ? 'color: #20a0ff' : ''" @click="jumpactNo(scope.row.actNo)"> {{ scope.row.actNo }}</a>
        </template>
      </el-table-column>
      <el-table-column align="center" prop="price" width="100"  label="价格"/>
      ...
      <el-table-column align="center"  prop="goodsName" label="商品名称">
        <template slot-scope="scope">
          <a :style="scope.row.goodsId ? 'color: #20a0ff' : ''" @click="jumpGoods(scope.row.goodsId , 1)"> {{ scope.row.goodsName }}</a>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    changeStatus(id, dataStatus) {
      goodsActApi.changeStatus({ id, dataStatus }).then(res => {
        if (res.subCode === 1000) {
          this.$message.success(res.subMsg)
        } else {
          this.$message.error(res.subMsg)
    ...
    batchdelete() {
      ...
      this.$confirm('是否继续?', '提示', {
        ...
        goodsActApi.batchdelete(this.ids).then(res => {
          if (res.subCode === 1000) {
            this.$message.success(res.subMsg)
            this.getPage()
          } else {
    ...
    exportHandle() {
      ...
      getExport('/gw/op/v1/goodsAct/export', data, 'post', '活动列表').then(() => {
        this.$emit('refresh')
      })
    },
    resetHandle() {
