...
<script>
  ...
  export default {
    ...
    methods: {
      getDetailById(id) {
        if (id) {
          goodsBusinessApi.getDetailById(id).then(res => {
            if (res.subCode === 1000) {
      ...
      goBack() {
        // *** 根据真实路径配置地址
        this.$router.push({path: '/goodsOrder/goodsBusiness'})
      },
      goEdit() {
        this.type = 2
      },
      submit() {
        this.$refs['form'].validate(async(valid) => {
          ...
          if (this.type == 2) {
            goodsBusinessApi.update(this.form).then(res => {
              if (res.subCode === 1000) {
            ...
            goodsBusinessApi.add(this.form).then(res => {
              if (res.subCode === 1000) {
                this.$message.success('操作成功')
                this.goBack()
              } else {
