<template>
  <div class="page-container">
    ...
    <el-row class="form-flex">
      ...
      <el-col :span="22">{{userInfo.staffType == 1 ? '正式' : userInfo.staffType == 0 ? '试用' :
        '--'}}
      </el-col>
    </el-row>
    <el-row class="form-flex">
    ...
    <el-row class="form-flex">
      ...
      <el-col :span="22">{{userInfo.dataStatus == 1 ? '正常' : userInfo.dataStatus == 0 ? '停用' :
        userInfo.dataStatus == -1 ? '删除' : '--'}}
      </el-col>
    </el-row>
    <el-row class="form-flex">
    ...
    <div class="login-ip-sty">
      <el-table height="600" border :data="tableData" class="table"
  ...
  export default {
    ...
    methods: {
      getOpSysUserAccountInfoById() {
        systemContainerApi.getOpSysUserAccountInfoById(
          { id: localStorage.getItem('user_id') }).then(res => {
          if (res.subCode === 1000) {
