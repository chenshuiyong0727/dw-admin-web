...
service.interceptors.request.use(
  config => {
    ...
    if (store.getters.token ? store.getters.token : getCookieByName('org_token_auth')) {
    ...
    let tokenExpireTime = sessionStorage.getItem('tokenExpireTime')
    ...
    if (tokenExpireTime && isTokenExpire(tokenExpireTime) === 1 && config.url !== '/gw/op/v1/auth/token/refresh') {
      if (!isTokenRefreshing) {
        ...
        let refreshParam = {
          ...
          refreshToken: getCookieByName('refresh_org_token_auth')
        }
        userContainerApi.refresh(refreshParam).then(res => {
          if (res.subCode === 1000) {
            setCookieByName('org_token_auth', res.data.token)
            store.dispatch('user/refreshNewToken', res.data.token)
            sessionStorage.setItem('tokenExpireTime', res.data.tokenExpireTime)
            config.headers['tokenAuth'] = getCookieByName('org_token_auth')
            reloadSubsubscribeArr(getCookieByName('org_token_auth'))
          } else {
